    ...
    public class Publisher : IPublisher
    {
        ...
        public async Task PublishAsync<T>(MQType mqType, string queueName, string actionName, T? data)
        {
            await _mqProducerFactory.Create(mqType)
                .SendAsync(queueName, _messageConverter.Serialize(new MessageBody { ActionName = actionName, NeedReply = false, Payload = data }));
        }
        ...
        public async Task<TR?> PublishAndWaitReplyAsync<T, TR>(MQType mqType, string queueName, string actionName, T? data)
        {
            var replyMessage = await _mqProducerFactory.Create(mqType)
                .SendAndWaitReplyAsync(queueName, _messageConverter.Serialize(new MessageBody { ActionName = actionName, NeedReply = true, Payload = data }));
        ...
        {
            await _mqProducerFactory.Create(mqType)
                .SendMulticastAsync(queueName, _messageConverter.Serialize(new MessageBody { ActionName = actionName, NeedReply = false, Payload = data }));
        }
